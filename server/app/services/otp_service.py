...
def generate_otp_service_recovery_password(email):
    ...
    subject = "Mã OTP khôi phục mật khẩu / Password Recovery OTP - Book Shop"
    ...
    <html>
      <body style="font-family: Arial, sans-serif; line-height: 1.6;">
        ...
        <p>Xin chào <strong>{email}</strong>,</p>
        <p>Bạn hoặc ai đó đã yêu cầu đặt lại mật khẩu cho tài khoản tại <strong>Book Shop</strong>.</p>
        ...
        <p>Hello <strong>{email}</strong>,</p>
        <p>You (or someone else) requested to reset the password for your <strong>Book Shop</strong> account.</p>
